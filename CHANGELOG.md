...
### Fixed
- Extracted text parts from Gemini API response directly to avoid non-text parts warnings in CLI output (caused by `thoughtSignature` blocks in newer Gemini models)

## [2.7.2] - 2026-06-13

### Changed
- Updated Google Gemini model to `gemini-3.1-flash-lite` (cheapest active Gemini model)
- Updated Anthropic Claude model to `claude-haiku-4-5` (active Claude Haiku model)
- Updated CLI interactive provider selection display names to match new models
